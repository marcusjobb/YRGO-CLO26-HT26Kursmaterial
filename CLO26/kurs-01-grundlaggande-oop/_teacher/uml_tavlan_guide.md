# Lärarguide — UML på tavlan

*Rita det här på tavlan INNAN du öppnar VS Code.*  
*Låt dem sitta med pennor och kopiera — det tar 3 minuter och de minns det.*

---

## UML-klassdiagram att rita
...
> "Och ser ni minustecknen? Det betyder att ingen annan kod kan röra `saldo` direkt.
> Den enda som får ändra saldo är klassen själv — via `SättIn` och `TaUt`.
...
> "Om `saldo` vore publikt — vad skulle kunna gå fel?"

Vänta på svar. Någon säger "man kan sätta den till vad som helst".  
Exakt. Skriv det på tavlan: `konto.saldo = 1000000;`  
